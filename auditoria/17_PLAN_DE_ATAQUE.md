...
| 8 bis | **Vaciar los estilos en línea** | VIS-001 (resto) | Sacar los 288 `setStyleSheet` a la hoja central y retirar las capas `legacy_styles` (25 ficheros) y `tema_aplicacion` (14). Es trabajo por vista, con revisión visual de cada una | P2 | alto | L | 8 | ratchet `setStyleSheet` bajando por lotes; capturas antes/después | pendiente |
| ~~9~~ | ~~**Componentes y feedback**~~ ✅ **v5.73.0** | ~~VIS-004~~, ~~VIS-006~~, ~~VIS-007~~, ~~UXA-003~~, ~~UXA-013~~, VIS-005 (descartado), VIS-008 (parcial) | Cabecera única; jerarquía de botones; avisos accesibles con errores que esperan; emojis fuera de los controles. El terminal retro se conserva por decisión de producto | P2 | medio | L | 8 | `test_componentes_y_feedback.py` (12 tests) ✅ | **completado** |
| ~~10~~ | ~~**Clics y caminos dorados**~~ ✅ **v5.71.0** | ~~UXF-003~~, ~~UXF-006~~, ~~UXF-009~~, ~~UXF-010~~, ~~UXF-011~~ | Carpeta recordada; jerarquía entre Generar y Limpiar; Ctrl+1…Ctrl+0 anunciados; GP-1 de 5 a 3 clics; deshacer sustituciones | P2 | bajo | M | 9 | `test_flujo_y_atajos.py` (17 tests) ✅ | **completado** |
...
| 12 | **Escalabilidad de datos** | ~~ESC-001~~, ~~UXA-011~~ (descartados por medición), ESC-005, UXA-009 | Medido: 17 ms el calendario con 2.800 guardias, 13 ms profesores con 200, 25 ms con 1.000. El refactor a `QTableView` no compraba nada. Quedan la caché por curso y los gráficos accesibles | P2 | medio | L | 9 | `test_escalabilidad_vistas.py` como banco de pruebas ✅ | **parcial v5.66.0** |
| 13 | **Solver escalable** | ~~ESC-002~~ (parcial), FUN-013, FUN-014 | Hilos = núcleos del equipo y tiempo máximo configurable ✅. Descomposición y diagnóstico por slot pendientes, y sólo tienen sentido si aparece un caso que no resuelva en 120 s | P2 | medio | L | 1 | `tests/compliance` + banco de pruebas | **parcial v5.66.0** |
