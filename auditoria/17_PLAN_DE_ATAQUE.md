...
1. **Lote 0** en la máquina Windows: compilar con `scripts/build_windows.ps1 -Diagnostico` y ejecutar el protocolo de [[06_CRASH_WINDOWS_GENERACION]] §5. Es ahora el paso de mayor valor: con los lotes 1 y 2 cerrados, **todos los hallazgos CRW menos CRW-003 están resueltos**. Si el cierre desaparece, el problema estaba en la frontera solver↔Qt; si persiste, la causa es CRW-003 (la sesión compartida) y el lote 3 pasa a ser urgente en lugar de preventivo.
2. **Lote 3** (sesión por hilo, CRW-003 + ESC-003) — el único hallazgo CRW que queda vivo y la dependencia de los lotes 6 y 7. Es el más caro del bloque (esfuerzo L, riesgo alto: toca 10 vistas) y conviene hacerlo por partes, empezando por generación y sync. Nota heredada del lote 2: `_sincronizar` ya no bloquea la interfaz, pero sigue pasando la sesión de la GUI al `SyncWorker`; ése es exactamente el nudo que desata este lote.
3. **Lote 4** (resto de la suite: `make venv`, `xfail_strict`, tests del formulario muerto) si se quiere un frente barato e independiente mientras tanto.
