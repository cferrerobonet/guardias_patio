...
| Capas de estilo | 4 ficheros (light.qss 348 líneas, tema_aplicacion.py 822, legacy_styles.py 308, ui_styles.py 391) + inline | Cuatro fuentes de verdad |
...
| --- | --- | --- | --- | --- |
| VIS-001 | P2 | Cuatro capas de estilo compiten: `light.qss` aplicado en `QApplication`, `get_complete_stylesheet()` en la ventana, `legacy_styles`/`ui_styles` y 287 inline | `main.py:168-176`, `ventana_principal.py:87`, `theme/legacy_styles.py`, `ui_styles.py` | Un único `app.qss` generado desde `tokens.py`; inline prohibido salvo `objectName`/propiedades |
...
| VIS-006 | P2 | Títulos inconsistentes: "CÁLCULO Y ASIGNACIÓN" en mayúsculas, otros en Title Case, y el título registrado no se pinta | `asignacion_calculo_form.py:78`, `ventana_principal.py:45-68` | Cabecera común de vista (título + subtítulo + acciones) renderizada por `ContentWrapper` |
...
| VIS-009 | P2 | Tamaño mínimo 1400×900 en ventana vs 1200×800 en settings; portátiles 1366×768 quedan fuera | `ventana_principal.py:84`, `settings.py:109-110` | Mínimo 1024×700 con scroll interno; tamaño recomendado desde `availableGeometry` |
| ~~VIS-010~~ ✅ **v5.83.0** | P3 | ~~Identidad: tema y componentes con nombre prestado; branding EPLA inconsistente~~ · renombrados a `ventana_principal`, `menu_lateral` y `tema_aplicacion`; el logo del centro ya estaba en menú lateral, PDF y avisos | `presentation/themes/tema_aplicacion.py`, `components/menu_lateral.py` |
