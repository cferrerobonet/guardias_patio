...
| SYNC-014 | P2 | alta | ~~La sincronización automática solo sube y su fallo pasa desapercibido~~ | `ccleaner_main_window.py`, `sync/sync_manager.py` | **RESUELTO v5.69.0** · **descargar en mitad de la sesión queda descartado a propósito**: cambiaría los datos bajo las vistas abiertas, y el modelo acordado es «uno cada vez» con la copia buena traída al arrancar. Lo que sí era un problema es que el rechazo de la subida sólo se veía en el registro y en un «✕ Error de sync» diminuto: ahora el gestor recuerda el motivo, el indicador lateral dice «Sin subir: la nube cambió» y se explica una vez por sesión qué ha pasado y qué hacer. Un corte puntual de red no interrumpe · `tests/audit/test_aviso_de_sincronizacion.py` | 12 |
