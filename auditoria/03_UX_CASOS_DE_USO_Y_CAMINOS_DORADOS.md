...
| --- | --- | ---: | ---: | --- |
| GP-1 Generar guardias (curso configurado) | Sidebar Cálculo (1) → Generar (1) → Sidebar Calendario (1). Desde v5.56.0 no hace falta «Calcular cuotas» antes, y desde v5.71.0 no hay modal informativo previo y el progreso se cierra solo al acabar bien. Al **regenerar** son 4: se añade la elección entre «desde hoy» y «todo el curso», que es una decisión real, no ceremonia | **3** | 3 | Cuotas se recalculan automáticamente si los datos cambiaron; resumen previo inline (no modal); el diálogo de progreso se cierra solo al acabar y muestra tarjeta de resultado con botón "Ver calendario" |
...
| ~~UXF-006~~ ✅ **v5.71.0** (Generar pesa el triple que Limpiar) | P2 | "Limpiar guardias" comparte fila, tamaño y prominencia con "Generar" | `generacion_panel.py:165-176` |
