...
| ~~FUN-009~~ ✅ **v5.93.0** | ~~Consulta web de solo lectura para el profesorado~~ · resuelto con **páginas estáticas**, no con la API: una página y un `.ics` por profesor, con dirección propia estable y no adivinable, para subir al hosting del centro. Decisión de CarlosFB (2026-09-06): la vía de la API exigía migrar a una base compartida y montar autenticación, y eso es el lote 17 | Alto a medio plazo | L | ESC-004, SEC-002 |
| FUN-010 | **Tema oscuro y alto contraste** generados desde tokens | Medio | S tras VIS-001 | VIS-001 |
| ~~FUN-011~~ ✅ **v5.81.0** | ~~Actualización automática en Windows y notas de versión in-app~~ · el aviso enseña las notas del release y pregunta antes de descargar; el instalador se abre con lo que entiende cada sistema —con `open` sólo funcionaba en macOS | Medio | S | BLD-005 |
...
| COD-004 | P3 | Código muerto: `DashboardForm`, `HomeForm`, `AsignacionGuardiasForm` (379 líneas, con tests propios), `ui_styles.py`, `legacy_styles.py` parcial, loggers a módulos inexistentes, `.spec` duplicados | `ventana_principal.py:139-166` vs `presentation/forms/__init__.py` | Borrar con sus tests o registrar en navegación; `vulture` como gate |
...
| COD-008 | P3 | Módulos > 700 líneas: `tema_aplicacion.py` 822, `db_manager.py` 820, `sync_manager.py` 815, `vista_calendario.py` 809, `profesor_form.py` 798, `generacion_panel.py` 797 | `wc -l` | Dividir por responsabilidad (`sync_manager`: backend, exportador, auth; `generacion_panel`: panel, presentador de resultados, notificador) |
...
| ESC-003 | P2 | Sync exporta toda la BD a JSON y la sube cada 30 min y al cerrar; crece linealmente; bloquea el cierre | `sync_manager.py:515-603`, `ventana_principal.py:262-270` | Export incremental por `updated_at` o subir el `.db` comprimido (`gzip`); sync sólo si hay cambios (hash); nunca en el hilo GUI |
