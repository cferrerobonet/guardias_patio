...
def profile_generar_calendario():
    ...
    with profile_operation(
        ...
        "profile_generar_calendario.prof"
    ):
...
def profile_calcular_distribucion():
    ...
    with profile_operation(
        ...
        "profile_calcular_distribucion.prof"
    ):
        session = SessionLocal()
        try:
...
def profile_listar_profesores():
    ...
    with profile_operation(
        "listar_profesores()",
        "profile_listar_profesores.prof"
    ):
        session = SessionLocal()
        try:
...
def profile_query_patterns():
    ...
    try:
        ...
        profesores_con_zonas = session.query(Profesor).join(
            Zona, Profesor.zona_preferida_id == Zona.id
        ).all()
        elapsed = time.time() - start
        ...
        guardias = session.query(Guardia).filter(
            Guardia.profesor_id == profesores[0].id
        ).all()
        elapsed = time.time() - start
...
def main():
    ...
    parser.add_argument(
        '--visualize',
        action='store_true',
        help='Abrir snakeviz después del profiling'
    )
