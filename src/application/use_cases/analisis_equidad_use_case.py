...
class AnalisisEquidadUseCase:
    ...
    def execute(self, request: AnalisisEquidadRequest) -> AnalisisEquidadResponse:
        ...
        try:
            ...
            cuotas_dto = self._mapear_cuotas_a_dtos(
                cuotas_esperadas,
                cuotas_asignadas,
                profesores
            )
            ...
            logger.info(f"✓ Análisis completado: índice={indice:.2%}, desbalances={len(desbalances)}")
    ...
    def _calcular_metricas(
        ...
        coef_var = desv_std / statistics.mean(desviaciones) if desviaciones and statistics.mean(desviaciones) > 0 else 0
    ...
    def _mapear_cuotas_a_dtos(
        ...
        for prof_id, esperada in cuotas_esperadas.items():
            ...
            if profesor:
                cuotas_dto.append(CuotaProfesorDTO(
                    profesor_id=prof_id,
    ...
    def _generar_recomendaciones(
        ...
        if not recomendaciones:
            recomendaciones.append("✅ La distribución tiene buena equidad, no se requieren ajustes.")
