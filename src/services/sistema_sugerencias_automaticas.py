...
class SistemaSugerenciasAutomaticas:
    ...
    def generar_paquetes_sugerencias(
        self, diagnostico: DiagnosticoCompleto
    ) -> List[PaqueteCambios]:
        """
    ...
    def _generar_sugerencias_profesores_sin_guardias(
        self, problema: ProblemaDetectado
        ...
        profesores_inactivos = self.db.query(Profesor).filter(Profesor.activo.is_(False)).all()

        candidatos = [
            p for p in profesores_inactivos if _turno_validator.es_compatible(p.turno, turno)
        ]
                ...
                cambios.append(
                    CambioSugerido(
                        ...
                        detalles={
                            ...
                            "turno": prof.turno,
                            "zona_preferida": prof.zona_preferida.nombre_zona
                            if prof.zona_preferida
                            else "Todas",
                        },
                    )
                )
        ...
        profesores_otros_turnos = self.db.query(Profesor).filter(Profesor.activo.is_(True)).all()

        candidatos_ampliar = [
            p for p in profesores_otros_turnos if not _turno_validator.es_compatible(p.turno, turno)
        ]
    ...
    def _generar_sugerencias_slots_vacios(
        ...
        if huecos_turno:
            ...
            candidatos = [p for p in profesores_inactivos if turno in (p.turnos or ["mañana"])][:3]
        ...
        if huecos_zona:
            ...
            if zona_obj:
                ...
                profesores_sin_zona = (
                    self.db.query(Profesor).filter(Profesor.activo.is_(True)).all()
                )

                candidatos = [
                    p
                    for p in profesores_sin_zona
    ...
    def aplicar_cambios(
        self, paquete: PaqueteCambios, cambios_seleccionados: Optional[List[int]] = None
    ) -> Dict:
        """
        ...
        resultados = {"aplicados": 0, "fallidos": 0, "detalles": []}
        ...
        for cambio in cambios_a_aplicar:
            try:
                ...
                resultados["detalles"].append(
                    {"cambio": cambio.descripcion, "exitoso": True, "resultado": resultado}
                )
            except Exception as e:
                resultados["fallidos"] += 1
                resultados["detalles"].append(
                    {"cambio": cambio.descripcion, "exitoso": False, "error": str(e)}
                )
