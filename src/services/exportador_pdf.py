...
class ExportadorPDF:
    ...
    def exportar_mes_consolidado(
        ...
        return exportar_mes_consolidado(session, mes, anio, ruta_salida, progress_callback)
    ...
    def exportar_curso_completo(
        ...
        return exportar_curso_completo(
            session, anio_inicio, carpeta_salida, profesor_ids, progress_callback
        )
    ...
    def exportar_profesor_individual_optimizado(
        ...
        from services._pdf_individual_optimizado import exportar_profesor_individual_optimizado

        return exportar_profesor_individual_optimizado(
            session, profesor_id, fecha_inicio, fecha_fin, ruta_salida, progress_callback
        )

    # ----- MÉTODO INTERNO (no extraído — delega en exportar_mes_consolidado) -----
    # Los métodos exportar_mes_consolidado, exportar_curso_completo y
    # exportar_profesor_individual_optimizado han sido extraídos a:
    #   services/_pdf_mes_consolidado.py
    #   services/_pdf_individual_optimizado.py
    # Los wrappers de arriba mantienen la API pública intacta.
